...
if ffi.os == "Windows" then
    ...
    function M.send(s, data, flags)
        local len = #data

        if not data then
            M._socket_lib.WSASetLastError(M.WSAErrorCodes.WSA_INVALID_PARAMETER)
        ...
        return M._socket_lib.send(s, p_data, len, flags)
    end


    function M.shutdown(s, how)
        local err_code = M._socket_lib.shutdown(s, how)
    ...
    function M.closesocket(s)
        local err_code = M._socket_lib.closesocket(s)
